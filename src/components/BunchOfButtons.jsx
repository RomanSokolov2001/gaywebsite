...
const Buttons = () => {
    return (
        Object.values(soundMap).map(file => {
            return (
           
                // <SoundButton file={file} text={getNameFromPath(file)}/>
                <Button sound={file} text={kebabToTitleCase(getNameFromPath(file))}/>
            )
...
const Button = (props) => {
    const { currentSound, setCurrentSound, isSoundsOverlapping, isEditMode, setFavorites, favorites } = useContext(AppContext);
    ...
    const handleClick = () => {
        setCurrentSound(props.text)
        setIsClicked(true);

        if (!isSoundsOverlapping) {
            stop()
            play()
      ...
      useEffect(() => {
        if (currentSound !== props.text && !isSoundsOverlapping) {
          stop();
        }
      }, [currentSound, props.text, isSoundsOverlapping, stop]);
    
    return (
        <div className={'bg-slate-100 rounded-xl m-1'}>
        <button
          className={`h-[45px] w-[45px] font-bold rounded-xl border text-[10px] border-black flex justify-center items-center transition-transform duration-10 ease-in ${isClicked ? 'scale-90' : ''}`}
          onClick={handleClick}
          style={{ pointerEvents: isClicked ? 'none' : 'auto' }}
        >
          
            {kebabToTitleCase(props.text)}
